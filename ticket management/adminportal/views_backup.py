...
def tickets_view(request):
    ...
    if request.GET.get('title'):
        tickets = tickets.filter(title__icontains=request.GET.get('title'))
    if request.GET.get('creator'):
        tickets = tickets.filter(creator_id=request.GET.get('creator'))
    if request.GET.get('client'):
        tickets = tickets.filter(client_id=request.GET.get('client'))
    if request.GET.get('technician'):
        tickets = tickets.filter(technician_id=request.GET.get('technician'))
    ...
    if request.GET.get('deadline'):
        tickets = tickets.filter(deadline__lte=request.GET.get('deadline'))
    if request.GET.get('created'):
        tickets = tickets.filter(created_at__date__lte=request.GET.get('created'))
...
def reject_ticket(request, ticket_id):
    ...
    return redirect('adminportal:approvals')


def approvals_view(request):
    status_filter = request.GET.get('status')
    if status_filter:
        tickets = Ticket.objects.filter(status=status_filter).order_by('-created_at')
    else:
        tickets = Ticket.objects.filter(status__in=['pending_approval', 'completed', 'rejected']).annotate(
            status_order=Case(
                When(status='pending_approval', then=Value(0)),
    ...
    # Get active technicians for the dropdown
    technicians = User.objects.filter(role='employee', status='active')
    
    return render(request, 'adminportal/approvals_list.html', {
        'tickets': tickets,
        'technicians': technicians
    })
...
def system_settings_view(request):
    ...
    if request.method == "POST":
        setting.system_name = request.POST.get("system_name", setting.system_name)
        if request.FILES.get("system_logo"):
            setting.system_logo = request.FILES["system_logo"]
        setting.theme = request.POST.get("theme", setting.theme)
        setting.default_priority = request.POST.get("default_priority", setting.default_priority)
        setting.default_status = request.POST.get("default_status", setting.default_status)
        setting.working_hours = request.POST.get("working_hours", setting.working_hours)
        setting.timezone = request.POST.get("timezone", setting.timezone)
        setting.date_format = request.POST.get("date_format", setting.date_format)
        ...
        setting.password_policy = request.POST.get("password_policy", setting.password_policy)
        setting.maintenance_mode = bool(request.POST.get("maintenance_mode"))
        setting.save()
        messages.success(request, "General settings updated successfully.")
        return redirect("adminportal:system_settings")
    context = {"setting": setting}
    return render(request, "adminportal/settings.html", context)
